...
第二部分目标：设计数据库表，编写数据库连接池，编写实体类以及DAO层类，
3.编写ConnectionPool类
功能：
3.1,读取数据库配置
3.2,保存有数据库连接队列，提供数据库连接获取接口。
3.3,管理数据库连接的动态增加和回收，删除
4.编写Connection类
功能：
4.1,提供数据库连接接口，供连接池使用
...
8.编写网络类clientNet
功能
8.1，主要是连接服务器，发送数据，接收数据。供上层服务使用

9，编写showUI类
功能：
9.1，在这里显示所有界面相关的显示和输入操作
9.2，通过输入的命令，调用服务类的服务，如果登录，注册，聊天，添加好友群组等等。
9.3，登录成功后，新建线程调用服务类的收数据服务。显示新消息。

10，编写clientservice类。
功能：
10.1，保存当前登录用户的状态，供上层showUI类显示调用
10.2，提供服务给上层的showUI类
10.3，对收到的数据进行合法性验证以及序列化数据发送，调用网络类进行发送和接收数据

第四部分目标：修改单服务器为集群服务器
思路：
使用nginx反向代理，用户在登录时，根据配置的方式，如轮询地，从服务器集群中选择服务器登录。
用A在M服务器登录时，把在线状态存入redis中，并向redis订阅该用户的名称的频道
...
第五部分，编写rpc通信框架，对服务端的服务类的一些服务进行包装，提供远程调用接口。
